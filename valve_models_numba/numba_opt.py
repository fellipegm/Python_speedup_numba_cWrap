...
@njit
def initalize_simdata_numba(simdata, len: int) -> np.ndarray:
...
@njit
def upsample_numba(x_init: float, x_end: float, nSamp: int, return_data: np.ndarray) -> np.ndarray:
...
@njit
def sim_karnopp_numba(u: np.ndarray, Ts: float, 
    ...
    simdata = numba.typed.Dict.empty(
        key_type=types.unicode_type,
        value_type=float_array,
    )
    simdata = initalize_simdata_numba(simdata, len)
    ...
    for i in range(len):
        ...
        P_us = upsample_numba(u[i], u[i+1], nSamp, P_us)
